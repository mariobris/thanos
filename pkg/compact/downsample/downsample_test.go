...
// testDownsample inserts the input into a single series block and invokes the downsampler with the given resolution.
func testDownsample(t *testing.T, d *downsampleTestCase, resolution int64) {
	...
	if len(d.inRaw) > 0 {
		fakeMeta.Thanos.Downsample.Resolution = 0
		for _, samples := range d.inRaw {
		...
		fakeMeta.Thanos.Downsample.Resolution = resolution - 1
		for _, chk := range d.inAggr {
			ser.chunks = append(ser.chunks, encodeTestAggrSeries(chk))
		}
	}
	mb.addSeries(ser)

	id, err := Downsample(logger, fakeMeta, mb, dir, resolution)
	testutil.Ok(t, err)
	...
	testutil.Equals(t, d.expected, got)
}
...
var (
	// Decoded excerpt of pkg/query/testdata/issue2401-seriesresponses.json
	realisticDuplicatedChkDataWithStaleMarkers = [][]sample{
		{
...
func TestCounterSeriesIterator(t *testing.T) {
	...
	for _, tcase := range []struct {
		...
		},
		{
			...
			chunks: realisticDuplicatedChkDataWithStaleMarkers,
			expected: []sample{
